resource "aws_alb_target_group" "data-app-backend-target-api" {
...
resource "aws_alb_target_group" "data-app-backend-target-web" {
    # name= "${var.application_name}-alb-target-80" # no name allows terraform to destroy and recreate with new name.
    protocol = "HTTP"
    port = 80
    target_type = "ip" # flexible targeting as opposed to fixed resource (instance id)
...
resource "aws_alb_listener" "data-app-web-listener" {
  region = var.region
  ...
  default_action {
    type = "forward"
    target_group_arn = aws_alb_target_group.data-app-backend-target-web.arn
  }
...
resource "aws_alb_listener" "data-app-frontend-listener" {
  ...
  default_action {
    type = "forward"
    target_group_arn = aws_alb_target_group.data-app-backend-target-api.arn
  }
